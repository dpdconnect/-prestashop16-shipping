...
class DpdLabelGenerator
{
    ...
    {
        $url = Configuration::get('dpdconnect_connect_url');
        $username = Configuration::get('dpdconnect_connect_username');
        $encryptedPassword = Configuration::get('dpdconnect_connect_password');
        if ($encryptedPassword === null || $encryptedPassword === "") {
            throw new Exception('Missing credentials for DPD-Connect');
        }
    ...
    public function generateLabel($orderIds, $parcelCount, $return)
    {
        ...
        if (count($labelRequests)) {
            $labelResponses = $this->requestLabels($labelRequests);
            if ($this->errors) {
                return;
            }
    ...
    private function requestLabels($labelRequests)
    {
        $request = [
            'printOptions' => [
                'printerLanguage' => 'PDF',
    ...
    private function redirectToPdfDownload($label)
    {
        $pdf = $label['pdf'];
